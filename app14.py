...
# ===== THEME: Beige × Green =====
PALETTE = {
    "bg": "#F6F3EE",            # アプリ背景（温かいベージュ）
    "bg_secondary": "#EFE9DD",  # セクション/カード背景（ヘッダー含め統一）
    "primary": "#2A6E5B",       # 主色（ディープグリーン）
    "primary_hover": "#245E4D", # ボタンhover
    "accent": "#9CC5B0",        # 補助（ミント寄り）
    "border": "#D8D0C2",        # 枠線（淡いベージュ）
    "text": "#1F1F1F",          # 文字（濃いめグレー）
    "muted": "#6F6A62",         # サブテキスト
    "danger": "#C15555"         # 注意色
}

def inject_theme(p=PALETTE):
    css = f"""
    <style>
      /* 全体背景 & 文字色 */
      .stApp {{
      ...
      [data-testid="stSidebar"] .stButton > div > button:hover {{
        ...
        border: 1px solid {p["primary_hover"]} !important;
      }}

      /* チェック/ラジオ等 */
      [data-baseweb="checkbox"] input:checked + label > div {{
      ...
      /* ====== ヘッダー(白っぽさ解消)：Expander / Tabs / Forms のHeader面を統一 ====== */
      /* Expander 全体を統一の面色に */
      ...
      .stTabs [aria-selected="true"] button {{
        font-weight: 600;
        box-shadow: inset 0 -2px 0 {p["primary"]};
      }}
      ...
      /* border=True のコンテナ（カード風） */
      div:has(> div[data-testid="stHorizontalBlock"]) > div[style*="border: 1px solid"] {{
...
def login_ui():
    ...
    with c2:
        st.markdown(
            """
            <div style="text-align:center; margin-top:.25rem; margin-bottom:1rem;">
              <div style="font-size:2rem; font-weight:800; color:#1F1F1F;">Dietary</div>
              <div style="color:#6F6A62;">毎日の食事提案と記録をかんたんに</div>
            </div>
            """,
    ...
    with st.container(border=True):
        ...
        with col1:
            if st.button("ログイン", disabled=(not email or not pw), use_container_width=True):
                try:
        ...
        with col2:
            if st.button("新規登録", disabled=(not email or not pw), use_container_width=True):
                try:
...
REQUIRED_COLS_DEFAULTS = {
    ...
    "fat_g": 0.0,
    "carb_g": 0.0,
    "fiber_g": 0.0,    # ← DB には残す（ロジック/表示では使わない）
    "image_url": "",
    "url": ""
}
...
# ===============================
# 料理ジャンル判定（重複禁止＆主食必須に利用）
# ===============================
TERM_KEYWORDS = [
    ("noodle",   ["麺", "そば", "うどん", "パスタ", "ラーメン", "焼きそば"]),
    ("bento",    ["弁当"]),
    ("ricebowl", ["丼", "牛めし", "親子丼", "カツ丼", "海鮮丼", "中華丼"]),
    ("riceball", ["おにぎり", "おむすび"]),
    ("curry",    ["カレー"]),
    ("sushi",    ["寿司", "すし"]),
    ("bread",    ["パン", "サンド", "バーガ", "トースト", "ロール"]),
    ("soup",     ["スープ", "汁", "みそ汁", "味噌汁"]),
    ("salad",    ["サラダ"]),
    ("side",     ["惣菜", "おかず", "唐揚", "から揚げ", "コロッケ", "ハンバーグ", "焼き魚", "餃子", "シュウマイ", "小鉢"]),
    ("dessert",  ["デザート", "スイーツ", "プリン", "ゼリー", "ヨーグルト", "ケーキ", "シュー"]),
    ("drink",    ["ドリンク", "飲料", "コーヒ", "ラテ", "お茶", "ティー", "ジュース"]),
]
# 主食扱いのジャンル（最低1つ必要）
MAIN_TERMS = {"noodle", "bento", "ricebowl", "riceball", "curry", "sushi", "bread"}

def classify_term(category: str, name: str) -> str:
    text = f"{category} {name}".lower()
    for term, keys in TERM_KEYWORDS:
        for k in keys:
            if k.lower() in text:
                return term
    return "other"
...
# === コンボ検証（重複禁止＋主食必須） ===
def compute_term(item: dict) -> str:
    return classify_term(str(item.get("category","")), str(item.get("name","")))

def is_valid_combo_items(items: list) -> bool:
    """同一枠の中で：
    - term（料理ジャンル）が重複しない
    - MAIN_TERMS（主食）を少なくとも1つ含む
    """
    terms = [item.get("term") or compute_term(item) for item in items]
    if len(set(terms)) < len(terms):
        return False
    return any(t in MAIN_TERMS for t in terms)

# ===============================
# 活動量レベル（わかりやすい表示）
# ===============================
ACTIVITY_LEVELS = [
    {"key": "sedentary","label": "ほぼ運動しない","factor": 1.2,"hint": "デスクワーク中心。通勤や日常生活のみ","weekly": "週0〜1回・〜20分の軽い散歩など"},
    {"key": "light","label": "軽い運動","factor": 1.375,"hint": "軽い運動を時々行う","weekly": "週1〜3回・20〜40分（早歩き/ヨガ/軽い筋トレ）"},
    {"key": "moderate","label": "中程度の運動","factor": 1.55,"hint": "定期的に運動する","weekly": "週3〜5回・30〜60分（ジョギング/筋トレ）"},
    {"key": "active","label": "激しい運動","factor": 1.725,"hint": "ほぼ毎日トレーニング","weekly": "週6〜7回・45〜90分（高強度インターバル/競技スポーツ）"},
    {"key": "very_active","label": "非常に激しい","factor": 1.9,"hint": "二部練や重労働を伴う","weekly": "毎日2回以上/重労働（アスリート/建設現場など）"},
]
...
def generate_item_combos(df_slot, budget, max_items=3):
    """
    - 価格合計が予算以内
    - コンボ内で term の重複禁止（麺×麺、スープ×スープ、サイド×サイド 等）
    - コンボに MAIN_TERMS（主食）を最低1つ含む
    """
    df = df_slot.copy()
    df["term"] = df.apply(lambda r: classify_term(str(r.get("category","")), str(r.get("name",""))), axis=1)
    items = df.to_dict("records")

    out = []
    for r in range(1, min(max_items, len(items)) + 1):
        for comb in combinations(items, r):
            price = sum(x.get("price_jpy", 0) for x in comb)
            if price > budget:
                continue
            if not is_valid_combo_items(list(comb)):  # 🔒 厳格チェック
                continue

            out.append({
                "kcal":   sum(x.get("kcal", 0) for x in comb),
                "price":  price,
                "items":  comb,
                "protein": sum(x.get("protein_g", 0.0) for x in comb),
                "fat":     sum(x.get("fat_g",     0.0) for x in comb),
                "carb":    sum(x.get("carb_g",    0.0) for x in comb),
                "fiber":   sum(x.get("fiber_g",   0.0) for x in comb),  # 互換（評価・表示では未使用）
            })
    return out
...
def plan_score(
    ...
    p = plan["protein_total"]; f = plan["fat_total"]; c = plan["carb_total"]
    ...
    if p < p_min:
        ...
        score += w_p * over_penalty * (p - p_max)

    if f < f_min:
        score += w_f * (f_min - f)
    elif f > f_max:
        score += w_f * over_penalty * (f - f_max)

    if c < c_min:
        score += w_c * (c_min - c)
    elif c > c_max:
        score += w_c * over_penalty * (c - c_max)
...
def optimize_day_fixed_score_no_overlap(combos_b, combos_l, combos_d, intake, budget, weight_kg, rng=None):
    ...
    # 念のため候補自体も検証
    combos_b = [c for c in combos_b if is_valid_combo_items(list(c["items"]))]
    combos_l = [c for c in combos_l if is_valid_combo_items(list(c["items"]))]
    combos_d = [c for c in combos_d if is_valid_combo_items(list(c["items"]))]
    ...
    for cb in cands_b[:60]:
        if not is_valid_combo_items(list(cb["items"])):  # 保険
            continue
        names_b = names_set(cb)
        for cl in cands_l[:60]:
            if not is_valid_combo_items(list(cl["items"])):  # 保険
                continue
            ...
            fiber_bl = cb["fiber"] + cl["fiber"]  # 互換のため保持（評価には使わない）
            ...
            for cd in pool_d:
                if not is_valid_combo_items(list(cd["items"])):  # 保険
                    continue
                ...
                plan = {
                    ...
                    "fiber_total":   fiber_bl + cd["fiber"],  # 表示はしない
    ...
    K = min(15, max(5, len(candidates)//10))
    topK = candidates[:K]
...
def _render_slot_cards(slot_key: str, jp_title: str, best_plan: dict, target_kcal: int):
    ...
    st.markdown(
        ...
        f"P{combo['protein']:.0f}・F{combo['fat']:.0f}・C{combo['carb']:.0f}"
    )
...
def page_my_page():
    ...
    with st.expander("基礎条件入力", expanded=True):
        ...
        with st.expander("活動量レベルの目安（週あたりの運動イメージ）", expanded=False):
            df_lv = pd.DataFrame([{
                "レベル": lvl["label"], "係数": lvl["factor"], "週あたりの目安": lvl["weekly"], "具体像": lvl["hint"]
    ...
    with st.form("today_form", clear_on_submit=False):
        ...
        with col_store_b:
            st.selectbox("朝の店舗", STORE_OPTIONS, key="store_b_input",
                         index=_safe_index(STORE_OPTIONS, st.session_state.get("store_b_input","セブンイレブン")))
        with col_store_l:
            st.selectbox("昼の店舗", STORE_OPTIONS, key="store_l_input",
                         index=_safe_index(STORE_OPTIONS, st.session_state.get("store_l_input","ファミリーマート")))
        with col_store_d:
            st.selectbox("夜の店舗", STORE_OPTIONS, key="store_d_input",
                         index=_safe_index(STORE_OPTIONS, st.session_state.get("store_d_input","ほっともっと")))
    ...
    if make_clicked:
        ...
        if best:
            ...
            for slot in ("breakfast","lunch","dinner"):
                if not is_valid_combo_items(list(best[slot]["items"])):
                    st.error(f"{slot} の組合せが不正（主食なし/系統重複）でした。再生成してください。")
                    st.stop()

            st.subheader("提案結果（同一商品の重複なし／主食1つ以上／系統の重複なし）")
            ...
            def explode_slot(slot, jp):
                ...
                for it in best[slot]["items"]:
                    rows.append([
                        ...
                        it["protein_g"], it["fat_g"], it["carb_g"],
...
def page_ai_trainer(age, sex, height, weight_today, weight_goal, activity):
    ...
    if client is None:
        st.error("OpenAI APIキーが設定されていないため、AIトレーナー機能は利用できません。")
        return
    ...
    if prompt := st.chat_input("運動や健康について何でもお聞きください..."):
        ...
        activity_factor_value = float(match.group(1)) if match else 1.55
...
if nav == "マイページ":
    ...
    page_ai_trainer(
        st.session_state.get("form_age", 33),
        st.session_state.get("form_sex", "male"),
        st.session_state.get("form_height", 173),
        # ↓ weight_today の優先順位：セッション最新 → プロフィールの初期体重 → 70.0
        st.session_state.get("weight_today", st.session_state.get("form_weight_now", 70.0)),
        st.session_state.get("form_weight_goal", 65.0),
        # 活動量もアプリ全体の表示形式と統一（"軽い運動 (1.375)"など）
        st.session_state.get("form_activity", DEFAULT_ACTIVITY_DISPLAY),
    )
else:
    page_today_plan()
