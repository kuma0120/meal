...
def load_products_df():
    return pd.DataFrame(
        PRODUCTS,
        columns=["store","category","name","kcal","price_jpy","meal_slot_hint",
...
def page_my_page():
    ...
    if "profile_initialized" not in st.session_state:
        ...
        # UIデフォルト（提案ページ用）
        st.session_state.setdefault("store_b_input", "seven")
        st.session_state.setdefault("store_l_input", "familymart")
        st.session_state.setdefault("store_d_input", "hottomotto")
...
def page_today_plan():
    ...
    STORE_OPTIONS = ["seven", "familymart", "hottomotto"]
    ...
    with st.form("today_form", clear_on_submit=False):
        ...
        with col_store_b:
            st.selectbox("朝の店舗", STORE_OPTIONS, key="store_b_input",
                         index=STORE_OPTIONS.index(st.session_state.get("store_b_input","seven")))
        with col_store_l:
            st.selectbox("昼の店舗", STORE_OPTIONS, key="store_l_input",
                         index=STORE_OPTIONS.index(st.session_state.get("store_l_input","familymart")))
        with col_store_d:
            st.selectbox("夜の店舗", STORE_OPTIONS, key="store_d_input",
                         index=STORE_OPTIONS.index(st.session_state.get("store_d_input","hottomotto")))
    ...
    if make_clicked:
        ...
        df_all = load_products_df()
        store_b = st.session_state.get("store_b_input", "seven")
        store_l = st.session_state.get("store_l_input", "familymart")
        store_d = st.session_state.get("store_d_input", "hottomotto")

        df_b = df_all[(df_all["store"] == store_b) & (df_all["meal_slot_hint"].isin(["breakfast","any"]))].reset_index(drop=True)
        df_l = df_all[(df_all["store"] == store_l) & (df_all["meal_slot_hint"].isin(["lunch","any"]))].reset_index(drop=True)
        df_d = df_all[(df_all["store"] == store_d) & (df_all["meal_slot_hint"].isin(["dinner","any"]))].reset_index(drop=True)
        ...
        combos_b = generate_item_combos(df_b, budget=budget_today)
        combos_l = generate_item_combos(df_l, budget=budget_today)
        combos_d = generate_item_combos(df_d, budget=budget_today)
